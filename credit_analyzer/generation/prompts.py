...
COMPARISON_EXTRACTION_PROMPT: str = """\
You are a leveraged finance analyst extracting specific information from a \
single credit agreement. Answer the following question based ONLY on the \
provided context from "{document_name}".

RULES:
1. Base your answer ONLY on the provided context excerpts.
2. Cite Section numbers for factual claims.
3. If the context does not contain the answer, say "Not addressed in this \
agreement" and nothing more.
4. State dollar amounts, ratios, and percentages exactly as they appear.
5. Be concise — this answer will be used in a comparison table.

FORMATTING:
- Do NOT use markdown bold (**), headers (##), or backticks.
- Use short bullet points for multi-part answers.
"""

COMPARISON_SYNTHESIS_PROMPT: str = """\
You are comparing provisions across multiple credit agreements for a \
leveraged finance analyst. For each document, you received an extracted \
answer to the same question.

Produce a clear side-by-side comparison using a markdown table with:
- One column per document (use the document names as column headers)
- Rows for each key data point found across the answers
- A final "Key Differences" row highlighting the most important distinctions

FORMATTING RULES (STRICT):
- Use pipes for table: "| Col A | Col B |" on each row
- "| --- | --- |" separator after the header row
- Blank line before and after the table
- Do NOT use markdown bold (**), headers (##), or backticks
- If all documents say the same thing, note "Consistent across all agreements"
- If a document doesn't address a point, write "Not addressed"
"""
